#this is the display class which shows a visual representation of the network learning. Each circle is a biase on a node
#and each line is a weight between nodes. red means the weight or biase is negative and blue means positive
#The severity of the color represents the severity of the weight or biase. When there are too many nodes to fit the screen
#(for example the 784 input nodes for this example) The display will show the middle 20 nodes. At the end the display will show
#some examples of numbers the network classified incorrectly along with what the network thought it was. As you will notice
#the visual is quite caotic and not very productive from an improvement point of view, but quite fun to watch none the less.
#you will need pygame installed to use this class
